...
int main( int argc, char* argv [] )
{
   ...
   fifteen g = fifteen();
   fifteen h{ { 1, 3, 4, 12 }, { 5, 2, 7, 11 }, { 9, 0, 14, 10 }, { 6, 15, 13, 8 } } ;
   std::cout << f << "\n" << g;
   std::cout << f.hashvalue() << " " << g.hashvalue() << "\n";
   std::cout << f.issolved()  << " " << g.issolved() << "\n";
   std::cout << f.distance()  << " " << g.distance() << "\n";
   std::cout << h << h.distance() << " " << h.hashvalue() << "\n";
   #if 1
   auto dist = solve(f);
